...
## September 9: atlas pointer fixes and tabletop polish

Transparent hand padding intercepted the lower atlas and zoom controls. The tray now passes pointer events through its empty space; actual cards and the market remain interactive. Every printed route marker receives clicks over its full visible rectangle. The payment chooser no longer consumes the first click on a different route, and repositions when its route changes. Clicking a route after the first card draw explains “Draw one more card first” above that route while preserving the game rule.

Low-thinking browser QA verified direct claims through marker centers and endpoints, one-click switching between gray routes, payment deductions, lower-edge route clicks with a full hand, and zoom controls. The nested SVG badge scales on entry and exit; sampled exit transforms decreased from 0.909 through 0.675 before removal. Hand title tooltips are removed. Hand shadows no longer clip at the former scroll boundary. The status ticket centers its message and uses vertical turn text; all five player plaques use their player color without avatar strips. The illustrated six-row scoring reference sits at the end of the destination column. Desktop and portrait-tablet checks include five players and twelve tickets.

The atlas was freshly generated with built-in ImageGen against the measured city/coast guide. The 621 KB WebP uses direct texture coordinates, removing the piecewise texture warp responsible for distorted corners. All 36 city centers remain on land. Exact prompts, sources, and integration notes are in `artifacts/atlas-redraw/provenance.md`. Final Chrome fixture captures and animation/layout observations are in that directory. The fixture deliberately shows five carriage styles and a completed ticket; it is not a new played match. An in-app browser capture showed a yellow-route fill discrepancy that did not reproduce in Chrome; Chrome was used for final visual checks.

Validation: 50 tests / 1,708 assertions pass; lint, full Svelte/server typecheck (zero diagnostics), production build, and diff checks pass. Fresh production startup and opening ticket selection pass with no console errors or warnings. Detailed pointer evidence is appended to `artifacts/route-network/animation-b-audit.md`.

## September 9: payment pins, route symbols, and carriage repairs

Multi-option claims now fan colored location pins from the route's actual projected midpoint. Each pin shows carriage and locomotive icons with exact counts. The 72px circular targets stay fixed during hover; their corresponding cards lift while the rest desaturate. Pins scale from their common anchor, flip below northern routes, and stay inside desktop/tablet edges. The layout derives from the current option list so switching between routes with different payment counts cannot leave stale pins. Outside clicks and Escape dismiss the fan; clicking another route switches it directly.

Cards and unclaimed colored segments share the reference's eight symbols: flame, ring, hourglass, clover, spiral, droplet, diamond, and asterisk. Neutral routes remain unmarked. Completed destination artwork gently desaturates without fading its completion stamp or points.

The imported train meshes retain more roof/window detail and use coherent player-colored upper bodies with dark chassis. Removed the arbitrary pale triangle selection and global tilt that pitched north–south cars upward. Placement follows terrain with a gentle local roll. All eight models were checked at every segment: no vertex falls below terrain. One SVG mask, derived from projected model bounds, prevents route graphics from painting over carriages while keeping city labels readable. The fleet uses nine draw calls and seven geometries; the formatted model asset is 867 KB, 217 KB gzipped. No steady-state FPS claim is made from the startup frame sample.

Browser QA covered eight ordinary choices, pointer/focus previews, exact ordinary and mixed-locomotive deductions, switching and dismissal, and 768×1024 tablet bounds. The mixed six-car claim spent exactly four blue cards and two locomotives, six trains, and awarded 15 points. See `artifacts/payment-fan/qa.md`, `carriage-qa.md`, and screenshots. Validation: 50 tests / 1,708 assertions pass, full typecheck reports zero diagnostics, and lint/build pass.
